...
generators_new_10 = data(:,30+3+1);
generators_new = data(:,30+1);
...
mbound         = data(:,28+1);
generatorsbound_new_10 = data(:,31+1);
generatorsbound_new = data(:,31+3+1);
...
mtime           = data(:,29+1);
generatorstime_new_10 = data(:,32+3+1);
time_gen10_solve = data( :,36+1)
time_gen_solve = data( :,37+1)
...
disp('Generators_new');
print_bound(optimalbound, generatorsbound_new_10);

print_time(generatorstime_new_10);
...
[mean(generators) mean(generators_new_10) mean(generators_new)]
A = [mean(generatorsbound) mean(generatorsbound_new_10) mean(generatorsbound_new)]
[Y I] = max(A);
if I== 1
    disp('best method is the old one')
else if I ==2
    disp('best method is the 10_gen one')
    else
        disp('best method is the 9_gen one')
    end
end
sprintf('Difference to new_10 is %f' , mean(generatorsbound) - mean(generatorsbound_new_10))


sprintf('Runtime for generators_10 = %f and old_gen = %f', mean(time_gen10_solve), mean(time_gen_solve))

%     c = colorspiral;
if ~use_packing
    h1 = plot_hist(generators_new_10,      edges,[ 0.9526    0.5773    0.8113],'-');
    ...
    leg_str = {'GRD-gen','GRD','GRD-heur.','Fix et al.','Generators_new'};
    if use_m
        leg_str{end+1} = 'M';
